...
class ProxyTemplate
{
  public:
    ...
    R clusterConnectionManagementSchema(const Vbin32& data_)
    {
        cluster-connection::ManagementSchema managementSchema(data_);
        return functor(managementSchema);
    }
    ...
    {
        cluster-connection::ManagementSetupState managementSetupState(objectNum_, bootSequence_);
        return functor(managementSetupState);
    }
    
    
    R clusterConnectionManagementAgents(const Vbin32& data_)
    {
        cluster-connection::ManagementAgents managementAgents(data_);
        return functor(managementAgents);
    }
  private:
    F functor;
};
