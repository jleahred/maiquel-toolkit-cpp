...
void  process_begin_scope           (mtk::bhvm* bm);
void  process_end_scope             (mtk::bhvm* bm);
...
void Assign2(const std::string& val2assign, std::string& assign2);
mtk::Signal<const mtk::Alarm&> signal_alarma;
...
int main()
{
    ...
    try
    {
        ...
        signal_alarma.connect(&machine.signal_error);
        ...
        MTK_HVM_REGISTER_COMMAND(machine, cmd_bs        , "cmd:begin_scope" , process_begin_scope );
        MTK_HVM_REGISTER_COMMAND(machine, cmd_es        , "cmd:end_scope"   , process_end_scope   );



	//mtk::asocmem0  memory;  está definida  global

	memory.signal_error.connect(&ShowLog);
	memory.signal_assign_to.connect(&Assign2);
...
//        machine.insert_instruction("push double:(__void__)");
...
//        machine.insert_instruction("push bool:(__void__)");
...
//        machine.insert_instruction("nop");
        ...
        //  si hubo un error en compilación...
        if (string2execute.substr(0, 6) == "ERROR ")
        {
            std::cout << string2execute << std::endl;
            return -1;
        }
...
std::string             s_get_void_value        (void);
bool s_is_void                                  (const std::string& value);
...
void  process_not_equal  (mtk::bhvm* bm)
{
    ...
    if (s_is_void(first)  ||  s_is_void(second))
    {
std::cout << "not equal... first:" << first << "  second:" << second << std::endl;
        bm->exec_push(code_val_false);
    }
    else
    {
...
std::string             s_get_void_value        (void)
{
    return MTK_SS(code_type_void << code_val_void);
}

bool s_is_void(const std::string& __value)
{
    std::string type;
    std::string value;
    s_get_val_type_and_value(__value).assign(type, value);

    if (type == code_type_void  ||  value == code_val_void)
        return true;
    else
        return false;
}
...
void Assign2(const std::string& val2assign, std::string& assign2)
{
    mtk::tuple<std::string, std::string>  val2assign_type_value =  s_get_val_type_and_value(val2assign);
    mtk::tuple<std::string, std::string>  assign2_type_value =  s_get_val_type_and_value(assign2);

    if (assign2 == ""  ||  (val2assign_type_value._0 == assign2_type_value._0))
    {
        assign2 = val2assign;
    }
    else
...
void s_save_value_in_var(const std::string& var_name, const std::string& var_value)
{
    memory.write_var(var_name, var_value);
}


std::string s_get_value_from_var    (const std::string& var_name)
{
    std::string value = memory.read_var(var_name);
...
mtk::tuple<std::string, std::string> s_get_val_type_and_value  (const std::string& value)
{
    long unsigned till_type = value.find_first_of(':')+1;
    return mtk::make_tuple(value.substr(0, till_type), value.substr(till_type));
}


void s_def_variable(const std::string& var_name, const std::string& var_value)
{
    std::string type;
    std::string value;
