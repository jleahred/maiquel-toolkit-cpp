...
* This process will control the login and logout from clients
...
* On start, during 4 minutes, it will add to session list, the received keep alive (in order to let a hot start)
...
* Servers will send +pub_partial_user_list_serv2acs+ frequently in order to detect invalid users registered on one server
...
.. +acs_server+ will notify this with an error message (it could be a concurrence behavior)
