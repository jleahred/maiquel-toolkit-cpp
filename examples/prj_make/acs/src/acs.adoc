...
.. Ask for this +session_id+
.. Notify as +error+ with an +alarm+
.. Reject the request
.. In case it would be a valid +session_id+, +acs_server+ will publish +pub_partial_user_list_acs2serv+ for next requests
.. +acs_server+ will notify this with an error message

[source,{cpp}]
----
include::..//src/msg_acs_server.msg[]
----
