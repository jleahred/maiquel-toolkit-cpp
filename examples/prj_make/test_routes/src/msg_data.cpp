...
sub_route::sub_route (   const std::string&  _origin,   const std::string&  _dest,   const std::string&  _qe_name,   const std::string&  _subject)
    :     origin(_origin),   dest(_dest),   qe_name(_qe_name),   subject(_subject) 
...
void  sub_route::check_recomended(void) const
{

}

void sub_route::before_send(void) const
{
...
sub_ip_names::sub_ip_names (   const std::string&  _name,   const std::string&  _ip)
    :     name(_name),   ip(_ip) 
...
void  sub_ip_names::check_recomended(void) const
{

}

void sub_ip_names::before_send(void) const
{
...
std::ostream& operator<< (std::ostream& o, const sub_route & c)
{
    o << "{ "
        ...
        << "origin:"<<   c.origin << "  "        << "dest:"<<   c.dest << "  "        << "qe_name:"<<   c.qe_name << "  "        << "subject:"<<   c.subject << "  "
        << " }";
    return o;
};
...
YAML::Emitter& operator << (YAML::Emitter& o, const sub_route & c)
{
    o << YAML::BeginMap
        ...
        << YAML::Key << "origin"  << YAML::Value <<   c.origin        << YAML::Key << "dest"  << YAML::Value <<   c.dest        << YAML::Key << "qe_name"  << YAML::Value <<   c.qe_name        << YAML::Key << "subject"  << YAML::Value <<   c.subject
        << YAML::EndMap;
    return o;
};
...
void  operator >> (const YAML::Node& node, sub_route & c)
{


        node["origin"]  >> c.origin;
        node["dest"]  >> c.dest;
        node["qe_name"]  >> c.qe_name;
...
YAML::Emitter& operator << (YAML::Emitter& o, const sub_ip_names & c)
{
    o << YAML::BeginMap
        ...
        << YAML::Key << "name"  << YAML::Value <<   c.name        << YAML::Key << "ip"  << YAML::Value <<   c.ip
        << YAML::EndMap;
    return o;
};
...
void  operator >> (const YAML::Node& node, sub_ip_names & c)
{
...
{
    return (          a.origin ==  b.origin  &&          a.dest ==  b.dest  &&          a.qe_name ==  b.qe_name  &&          a.subject ==  b.subject  &&   true  );
};
...
{
    return (          a.name ==  b.name  &&          a.ip ==  b.ip  &&   true  );
};
...
void  copy (sub_route& c, const qpid::types::Variant& v)
    {  
...
//   field_type
                    ...
                    it = mv.find("or");
                    if (it== mv.end())
                        throw mtk::Alarm(MTK_HERE, "msg_build", "missing mandatory field origin on message sub_route::__internal_qpid_fill", mtk::alPriorCritic);
                    else
                        copy(c.origin, it->second);
                        //c.origin = it->second;
//   field_type

                    it = mv.find("dest");
                    if (it== mv.end())
                        throw mtk::Alarm(MTK_HERE, "msg_build", "missing mandatory field dest on message sub_route::__internal_qpid_fill", mtk::alPriorCritic);
                    else
                        copy(c.dest, it->second);
                        //c.dest = it->second;
//   field_type

                    it = mv.find("qen");
                    if (it== mv.end())
                        throw mtk::Alarm(MTK_HERE, "msg_build", "missing mandatory field qe_name on message sub_route::__internal_qpid_fill", mtk::alPriorCritic);
                    else
                        copy(c.qe_name, it->second);
                        //c.qe_name = it->second;
//   field_type

                    it = mv.find("sub");
                    if (it== mv.end())
                        throw mtk::Alarm(MTK_HERE, "msg_build", "missing mandatory field subject on message sub_route::__internal_qpid_fill", mtk::alPriorCritic);
                    else
...
//  field_type
        __internal_add2map(map, a.origin, std::string("or"));
//  field_type
        __internal_add2map(map, a.dest, std::string("dest"));
//  field_type
        __internal_add2map(map, a.qe_name, std::string("qen"));
//  field_type
        __internal_add2map(map, a.subject, std::string("sub"));
...
void  copy (sub_ip_names& c, const qpid::types::Variant& v)
    {  
        const std::map<qpid::types::Variant::Map::key_type, qpid::types::Variant> mv = v.asMap();
...
//   field_type
                    ...
                    it = mv.find("n");
                    if (it== mv.end())
                        throw mtk::Alarm(MTK_HERE, "msg_build", "missing mandatory field name on message sub_ip_names::__internal_qpid_fill", mtk::alPriorCritic);
                    else
...
//   field_type
                    ...
                    it = mv.find("i");
                    if (it== mv.end())
                        throw mtk::Alarm(MTK_HERE, "msg_build", "missing mandatory field ip on message sub_ip_names::__internal_qpid_fill", mtk::alPriorCritic);
                    else
                        copy(c.ip, it->second);
                        //c.ip = it->second;
...
//  field_type
        __internal_add2map(map, a.name, std::string("n"));
//  field_type
        __internal_add2map(map, a.ip, std::string("i"));
...
sub_route::sub_route (const qpid::types::Variant::Map&  mv)
    :  //   field_type
   origin(__internal_get_default((std::string*)0)),
//   field_type
   dest(__internal_get_default((std::string*)0)),
//   field_type
   qe_name(__internal_get_default((std::string*)0)),
//   field_type
...
sub_ip_names::sub_ip_names (const qpid::types::Variant::Map&  mv)
    :  //   field_type
   name(__internal_get_default((std::string*)0)),
//   field_type
   ip(__internal_get_default((std::string*)0)) 
    {
        copy(*this, mv);
        check_recomended ();  
    }
