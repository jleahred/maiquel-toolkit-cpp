...
#define SEND_REJECT(rq, __SIGNAL_TYPE__, __MSG_TYPE__) \
    if (last_confirmation().HasValue())    \
    {               \
        mtk::trd::msg::CF_XX cfxx(rq.invariant, last_confirmation().Get().market_order_id, rq.request_info.req_id, last_confirmation().Get().total_execs, serrors, mtk::admin::get_control_fluct_info());  \
        mtk::trd::msg::CF_XX_LS  rjxx(cfxx, last_confirmation().Get().market_pos);      \
        mtk::trd::msg::__MSG_TYPE__  _rj_(mtk::trd::msg::RJ_XX_LS(rjxx, last_request().Get().request_pos));  \
    ...
    {      \
        ...
        {      \
            mtk::trd::msg::sub_total_executions total_execs(0.,     \
                                                ...
                                                last_request().Get().request_pos.quantity );    \
            mtk::trd::msg::CF_XX cfxx(rq.invariant, MTK_SS(market_order_id), rq.request_info.req_id, total_execs, serrors, mtk::admin::get_control_fluct_info());  \
