...
def get__QE__direct_or_parent(class_properties) :
    if class_properties.has_key('QE'):
        return class_properties['QE']
    elif class_properties.has_key('I'):
        for class_name, class_info, __class_properties, send_code  in ALL_MESSAGES:
            if class_name == class_properties['I']:
                return get__QE__direct_or_parent (__class_properties)
        return 'ERROR get__QE__direct_or_parent'
    else:
        return 'ERROR get__QE__direct_or_parent'
...
"""
    ...
    if (${FIELD_NAME}.HasValue() == false)
        MTK_EXEC_MAX_FREC_S(mtk::dtSeconds(10)) // I know it's for all instances
                mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "check_recomended", MTK_SS("missing recomended field **$FIELD_NAME** on $class_name  " << *this), mtk::alPriorError));
...
def generate__internal_get_default_forward(class_name, class_info, class_properties, send_code):
    INITIALITATION = ''
    template = """
...
def generate__internal_get_default(class_name, class_info, class_properties, send_code):
...
"""
    ...
    if class_properties.has_key('I'):
        ...
        CONSTRUCTOR_PARAMS_DEBUG_INIT += Template('   ${FIELD_NAME}(__internal_get_default((${field_type}*)0)),\n').substitute(
                                                field_type = class_properties['I'],
                                                FIELD_NAME = class_properties['I']
                                                )
...
""").substitute(HEADER_GUARD = '__' +FILE_NAME + '__', 
    ...
    for class_name, class_info, class_properties, send_code in MESSAGES:
        ...
        if class_properties.has_key('QE')    or  is_message_not_submessage(class_properties):
            derived_info += """

            //  DERIVED INFO
"""        
            ##for p in re.findall('[^\$\{]*\$\{([^\}]*)', class_properties["QE"]):
            for p in re.findall('[^\$\{]*\$\{([^\}]*)', get__QE__direct_or_parent(class_properties)) :
                param_name = p.replace('.', '_').replace('(','').replace(')','')
                qpidmsg_get_address_params.append("const std::string& "+ param_name)
...
""").substitute(class_name=class_name)
    ...
    for class_name, class_info, class_properties, send_code  in ALL_MESSAGES:
        content_file_h += generate__internal_get_default_forward(class_name, class_info, class_properties, send_code)
        if QPID:
            if class_name.find("::IC_") == -1 : # at the moment is not supported submessages defined inside a message
                content_file_h += Template("""
...
inline void   copy(std::string& result, const qpid::types::Variant& v)
{
    ...
    {
        MTK_EXEC_MAX_FREC_S(mtk::dtHours(4))
            mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "msg parsing", MTK_SS("Expected utf8 string for " << s), mtk::alPriorError, mtk::alTypeNoPermisions));
        MTK_END_EXEC_MAX_FREC
    }
    result = s;
}

inline void   copy(mtk::msg::raw_string& result, const qpid::types::Variant& v)
{
...
template<typename T>
void   copy(mtk::nullable<mtk::list<T>>& result, const qpid::types::Variant& v)
{
    mtk::list<T> temp;
    copy(temp, v);
    result = temp;
    ...
    {
        qpid::types::Variant  vs(a);
        vs.setEncoding("utf8");
        map[key] = vs;
    }

    inline void __internal_add2map (qpid::types::Variant::Map& map, const mtk::msg::raw_string& a, const std::string& key)
    {
        map[key] = a.string;
    }
    ...
    inline void  __internal_add2map  (qpid::types::Variant::Map& map, const mtk::list<mtk::FixedNumber>& l, const std::string& key)
    {
        qpid::types::Variant::List list;
        
        mtk::list<mtk::FixedNumber>::const_iterator it = l.begin();
        while (it != l.end())
        {
            list.push_back(QPID_DESCOMPOSE_FIXED_NUMBER(*it));
            ++it;
        }
...
"""
    ...
    for class_name, class_info, class_properties, send_code  in ALL_MESSAGES:
        content_file_cpp += generate__internal_get_default(class_name, class_info, class_properties, send_code)
