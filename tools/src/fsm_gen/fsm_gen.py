...
def execute_dhp():
    pipe = os.popen( os.path.join(DHP_PATH, 'dhp') + ' ' 
                    + os.path.join(FSM_GEN_PATH, 'fsm_gen.gram ') +  sys.argv[1])
    ...
    for prop in properties:
        if len(prop)>0:
            if prop["visibility"] != 'phony':
                ...
                result += Template("""    virtual void set_${name}(const ${type}& ) {  throw mtk::Alarm(MTK_HERE, "property not valid in current status", mtk::alPriorError);  };\n""").substitute(name=prop['name'], type=prop["type"])
    return result
...
def get_code_from_macro(macro_name):
    if macro_name == 'LOGIC_ERROR':
        return 'ci->sig_alarm(mtk::Alarm(MTK_HERE, "logic error", mtk::alPriorError));'
...
for file in sys.argv[1:] :
    ...
    GENERATE_ON = os.path.join(GENERATE_ON, os.path.split(sys.argv[1])[0])
    ...
    AUTOGENERATED_MESSAGE = Template("""//  code generated by fsm_gen
