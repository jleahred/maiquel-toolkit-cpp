...
${OUTPUT_SIGNALS}
    ...
    //  special autogenerated output
    mtk::Signal<const mtk::Alarm&, bool&/*rethrow*/>    sig_exception_not_trapped;
    mtk::Signal< >                                      sig_status_changed;
...
${INPUT_METHODS}
    ...
    //  OUTPUT_ASYNCH
${OUTPUT_SIGNALS_ASYNCH}

    //  special autogenerated output
    mtk::Signal<const mtk::Alarm&, bool& /*rethrow*/>       sig_exception_not_trapped;
    mtk::async_signal_inmediate< >                          sig_status_changed;
...
def generate_header():
    ...
    for output in OUTPUTS_LIST:
        if len(output)> 0:
            if output["visibility"] != "private":
                OUTPUT_SIGNALS_ASYNCH += Template("""    mtk::async_signal_inmediate< """ + get_params_string(output["params"], True) + """ > ${name};\n""").substitute(name=output["name"])
    ...
    hf.write(Template(header_template).substitute(
                    ...
                    OUTPUT_SIGNALS_ASYNCH   = OUTPUT_SIGNALS_ASYNCH,
...
#define FSM_CATCH_RETHROW_AND_SIGNAL(__SUBJECT__, __DESCRIPTION__)    \\
                    ...
                    bool  rethrow = true;                       \\
                    ci->owner->sig_exception_not_trapped(mtkError, rethrow);          \\
                    if(rethrow)                                         \\
                        throw(mtkError);                                       \\
                } catch (std::exception& e) {                           \\
                    auto alarm = mtk::Alarm(                                 \\
                                MTK_HERE,                            \\
                                __SUBJECT__,                      \\
                                __DESCRIPTION__  +std::string(" exception ") + e.what(),\\
                                mtk::alPriorCritic);                    \\
                    bool  rethrow = true;                       \\
                    ci->owner->sig_exception_not_trapped(alarm, rethrow);          \\
                    if(rethrow)                                         \\
                        throw(alarm);                                       \\
                } catch (...) {                                         \\
                    auto alarm = mtk::Alarm(                                 \\
...
{
    status_common_info (${NESTED_NAMESPACES}${CLASSNAME}_dangerous_signals_not_warped* _owner, $SCI_CONSTR_PARAMS)
        :
        owner(_owner),
$SCI_CONSTR_INITALIZER
    ...
    ${NESTED_NAMESPACES}${CLASSNAME}_dangerous_signals_not_warped* owner;
...
"""
    ...
    for output in OUTPUTS_LIST:
        if len(output) > 0:
            if output["visibility"] != 'private':
                SCI_CONSTR_PARAMS += Template(""" mtk::async_signal_inmediate<${type}>& _${name},""").substitute(name=output['name'] , type=get_params_string(output['params'], True))
...
def get_code_from_macro(macro_name):
    if macro_name == 'LOGIC_ERROR':
        ...
        if MACROS.get(macro_name) == None:
            ...
            return  'fsmgen__' + '  &&  fsmgen__'.join(re.sub(' +&& +', ' ', macro_name).split(' '))
...
void $STATUS_NAME::$METHOD_NAME ( $PARAMS )
{
    ...
    FSM_CATCH_RETHROW_AND_SIGNAL(method_name, "exception fsm, rethrowed")
...
def get_ctor_code(class_name) :
    ...
    if CODE != '':
        result = 	Template("""
            ...
            FSM_CATCH_RETHROW_AND_SIGNAL("${class_name}_ctor", "rethrowed")""").substitute(CODE = CODE, class_name=class_name);
    ...
    result =  Template("""${NESTED_NAMESPACES}${CLASSNAME}_dangerous_signals_not_warped::${CLASSNAME}_dangerous_signals_not_warped($PARAMS)
{
    mtk::CountPtr<$private_namespace::status_common_info> ci = mtk::make_cptr(
                            new $private_namespace::status_common_info (this, $CSI_CALL_PARAMS $SIGNALS));
...
def get_public_signals_connected_to_warper():
    ...
    for out in OUTPUTS_LIST:
        if len(out)>0:
            if out['visibility'] != 'private':
                result += '    ptr->' + out["name"] + '.signal.connect(&' +  out["name"] + ');\n'
    return result[:-1]
...
$SIGNALS
    ...
    //  automatic generated
    ptr->sig_exception_not_trapped.connect(&sig_exception_not_trapped);
    ptr->sig_status_changed.signal.connect(&sig_status_changed);
}
