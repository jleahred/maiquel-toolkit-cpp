...
void RowImpl::Set(int param, const mtk::dtDateTime& dt)
{
	if (mDescrArea == 0)
		throw LogicExceptionImpl("Row::Set[Time]", _("The row is not initialized."));
	if (mDialect == 1)
    ...
    IBPP::Timestamp value (dt.GetYear()        .WarningDontDoThisGetInternal(),
			   dt.GetMonth()       .WarningDontDoThisGetInternal(),
			   dt.GetDay()         .WarningDontDoThisGetInternal(),
                           dt.GetHours()       .WarningDontDoThisGetInternal(),
			   dt.GetMinutes()     .WarningDontDoThisGetInternal(),
			   dt.GetSeconds()     .WarningDontDoThisGetInternal(),
			   dt.GetMilliseconds().WarningDontDoThisGetInternal()*10);

	SetValue(param, ivTimestamp, &value);
	mUpdated[param-1] = true;
}

void RowImpl::Set(int param, const mtk::dtTimeQuantity& tq)
{
	if (mDescrArea == 0)
		throw LogicExceptionImpl("Row::Set[Time]", _("The row is not initialized."));
	if (mDialect == 1)
    ...
    mtk::dtDecodedTimeQuantity dtq  = tq.GetDecodedTimeQuantity();
    IBPP::Time value (dtq.hours.WarningDontDoThisGetInternal(),
		      dtq.minutes.WarningDontDoThisGetInternal(),
		      dtq.seconds.WarningDontDoThisGetInternal(),
		      dtq.milliseconds.WarningDontDoThisGetInternal() * 10);
...
bool RowImpl::Get(int column, mtk::nullable<mtk::dtDateTime>& dt)
{
	if (mDescrArea == 0)
		throw LogicExceptionImpl("Row::Get", _("The row is not initialized."));

    IBPP::Timestamp getDateTimestamp;
    if (Get(column, getDateTimestamp)== true)
        return true;
    mtk::dtDateTime result( mtk::dtYear(getDateTimestamp.Year()), mtk::dtMonth(getDateTimestamp.Month()), mtk::dtDay(getDateTimestamp.Day()),
                            mtk::dtHours(getDateTimestamp.Hours()), mtk::dtMinutes(getDateTimestamp.Minutes()),
                            mtk::dtSeconds(getDateTimestamp.Seconds()), mtk::dtMilliseconds(getDateTimestamp.SubSeconds()/10) );
    dt = result;
    return false;
}

bool RowImpl::Get(int column, mtk::nullable<mtk::dtTimeQuantity>& tq)
{
	if (mDescrArea == 0)
		throw LogicExceptionImpl("Row::Get", _("The row is not initialized."));

    IBPP::Time getTime;
    if (Get(column, getTime)== true)
        return true;

    mtk::dtTimeQuantity result( mtk::dtHours(getTime.Hours()), mtk::dtMinutes(getTime.Minutes()),
                            mtk::dtSeconds(getTime.Seconds()), mtk::dtMilliseconds(getTime.SubSeconds()/10) );
    tq = result;
    return false;
}
...
bool RowImpl::Get(const std::string& name, mtk::nullable<mtk::dtDateTime>& dt)
{
	if (mDescrArea == 0)
		throw LogicExceptionImpl("Row::Get", _("The row is not initialized."));

    IBPP::Timestamp getDateTimestamp;
    if (Get(name, getDateTimestamp)== true)
        return true;
...
bool RowImpl::Get(const std::string& name, mtk::nullable<mtk::dtTimeQuantity>& tq)
{
	if (mDescrArea == 0)
		throw LogicExceptionImpl("Row::Get", _("The row is not initialized."));

    IBPP::Time getTime;
    if (Get(name, getTime)== true)
        return true;
