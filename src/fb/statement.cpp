...
void StatementImpl::Set(int param, const mtk::dtDateTime &dt)             //  JLE
{
	if (mHandle == 0)
		throw LogicExceptionImpl("Statement::Set[Time]", _("No statement has been prepared."));
	if (mInRow == 0)
...
void StatementImpl::Set(int param, const mtk::dtTimeQuantity& tq)             //  JLE
{
	if (mHandle == 0)
		throw LogicExceptionImpl("Statement::Set[Time]", _("No statement has been prepared."));
	if (mInRow == 0)
    ...
    IBPP::Time value (dtq.hours.WarningDontDoThisGetInternal(), dtq.minutes.WarningDontDoThisGetInternal(), dtq.seconds.WarningDontDoThisGetInternal(), dtq.milliseconds.WarningDontDoThisGetInternal() * 10);
	mInRow->Set(param, value);
}
...
bool StatementImpl::Get(int column, mtk::nullable<mtk::DateTime>& dt)
{
	if (mOutRow == 0)
		throw LogicExceptionImpl("Statement::Get", _("The row is not initialized."));

	return mOutRow->Get(column, dt);
}

bool StatementImpl::Get(int column, mtk::nullable<mtk::dtTimeQuantity>& tq)
{
	if (mOutRow == 0)
		throw LogicExceptionImpl("Statement::Get", _("The row is not initialized."));

	return mOutRow->Get(column, tq);
}
...
bool StatementImpl::Get(const std::string& name, mtk::nullable<mtk::DateTime>& dt)
{
	if (mOutRow == 0)
		throw LogicExceptionImpl("Statement::Get", _("The row is not initialized."));

	return mOutRow->Get(name, dt);
}

bool StatementImpl::Get(const std::string& name, mtk::nullable<mtk::dtTimeQuantity>& tq)
{
	if (mOutRow == 0)
		throw LogicExceptionImpl("Statement::Get", _("The row is not initialized."));

	return mOutRow->Get(name, tq);
}
