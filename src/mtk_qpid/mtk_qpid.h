...
#define  __MTK_CHECK_NON_DEPRECIATED_THIS(__MSG_TYPE__)     \
        if(__internal_handler__->__impl->depreciated_on.HasValue() ==  true)    \
        {           \
...
#define  __MTK_CHECK_DEPRECIATED_THIS(__MSG_TYPE__, __DEPRECIATED_DATE__)     \
        ...
        if(__internal_handler__->__impl->depreciated_on.HasValue()   &&    __internal_handler__->__impl->depreciated_on.Get()!= (__DEPRECIATED_DATE__))                     \
        {           \
            MTK_EXEC_MAX_FREC(mtk::dtMinutes(15))           \
                mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "depreciated"           \
                        , MTK_SS("previus non depreciated register on " << #__MSG_TYPE__ << "  with diferent depreciated date  "  <<           \
                          __internal_handler__->__impl->depreciated_on.Get()  <<  " != "  <<  __DEPRECIATED_DATE__)           \
                        , mtk::alPriorError));           \
            MTK_END_EXEC_MAX_FREC           \
        }           \
        else           \
            __internal_handler__->__impl->depreciated_on = mtk::make_nullable((__DEPRECIATED_DATE__));           \
    ...
    {    \
        mtk::CountPtr< mtk::handle_qpid_exchange_receiverMT<__MSG_TYPE__> > __internal_handler__ =      \
                                            mtk::get_from_factory<mtk::handle_qpid_exchange_receiverMT<__MSG_TYPE__> >(mtk::make_tuple(     __URL__, \
                                                                                                                                            ...
                                                                                                                                            __FILTER__, \
                                                                                                                                            mtk::QUEUE__DEFAULT_RECEPTOR_CONFIG));     \
        __MTK_CHECK_NON_DEPRECIATED_F(__MSG_TYPE__)     \
        __internal_handler__->__impl->signalMessage.connect(__FUNCT_RECEPTOR__);     \
...
#define MTK_QPID_RECEIVER_CONNECT_THIS(__HANDLER__, __URL__, __FILTER__, __MSG_TYPE__, __FUNCT_RECEPTOR__)  \
    {    \
        ...
        __internal_handler__->__impl->signalMessage.connect(this, &std::remove_reference<decltype(*this)>::type::__FUNCT_RECEPTOR__);     \
        (__HANDLER__) = __internal_handler__;     \
    }
...
#define MTK_QPID_RECEIVER_CONNECT_F__WITH_ADDRESS(__HANDLER__, __URL__, __ADDRESS__, __FILTER__, __MSG_TYPE__, __FUNCT_RECEPTOR__)  \
    {    \
        mtk::CountPtr< mtk::handle_qpid_exchange_receiverMT<__MSG_TYPE__> > __internal_handler__ =      \
                                            mtk::get_from_factory<mtk::handle_qpid_exchange_receiverMT<__MSG_TYPE__> >(mtk::make_tuple(__URL__, __ADDRESS__, __FILTER__, mtk::QUEUE__DEFAULT_RECEPTOR_CONFIG));     \
        __internal_handler__->__impl->signalMessage.connect(__FUNCT_RECEPTOR__);     \
        __MTK_CHECK_NON_DEPRECIATED_F(__MSG_TYPE__)     \
        (__HANDLER__) = __internal_handler__;     \
    }
...
struct mtkqpid_receiver
{
    ...
    mtkqpid_receiver(const t_qpid_url& _url, const t_qpid_address&  _address, const t_qpid_filter& _filter, mtk::t_qpid_exch_recept_conf  _qe_config)
        :  session (mtk::get_from_factory<mtkqpid_session>(_url))
         , address      (_address)
         , filter       (_filter)
         , qe_config    (_qe_config)
...
//--------------------------------------------------------------------
    ...
    {               \
        static auto qpid_sender = mtk::get_from_factory< mtk::mtkqpid_sender2 >(mtk::make_tuple(mtk::admin::get_url(__URL_FOR__), __MESSAGE__.get_qpid_address(), mtk::QUEUE__DEFAULT_SENDER_CONFIG));    \
...
class __internal_impl_handle_qpid_exchange_receiver   :  public mtk::SignalReceptor {
    mtk::non_copyable nc;

    public:
        explicit __internal_impl_handle_qpid_exchange_receiver(
                                                        const t_qpid_url& url,
                                                        const t_qpid_address& address,
                                                        const t_qpid_filter& filter,
                                                        const t_qpid_exch_recept_conf&  recept_qpid_config);
        ~__internal_impl_handle_qpid_exchange_receiver(void);

        Signal<const qpid::messaging::Message&>        signalMessage;
...
typedef  mtk::delete_later_wrapper<__internal_impl_handle_qpid_exchange_receiver>   handle_qpid_exchange_receiver;
...
template<>
inline mtk::CountPtr< mtk::mtkqpid_receiver> create_instance_for_factory (const mtk::tuple<t_qpid_url, t_qpid_address, t_qpid_filter, mtk::t_qpid_exch_recept_conf>& key, mtk::CountPtr<mtk::mtkqpid_receiver> result)
{
    result = mtk::make_cptr(new mtk::mtkqpid_receiver(key._0, key._1, key._2, key._3));
    return result;
}


template<>
inline mtk::CountPtr< mtk::mtkqpid_sender2> create_instance_for_factory (const mtk::tuple<t_qpid_url, t_qpid_address, t_qpid_exch_sender_conf>& key, mtk::CountPtr<mtk::mtkqpid_sender2> result)
{
    result = mtk::make_cptr(new mtk::mtkqpid_sender2(key._0, key._1, key._2));
    return result;
}
...
template<>
inline mtk::CountPtr< mtk::handle_qpid_exchange_receiver> create_instance_for_factory (
                                                    const mtk::tuple<t_qpid_url, t_qpid_address, t_qpid_filter, mtk::t_qpid_exch_recept_conf>& key,
                                                    mtk::CountPtr< mtk::handle_qpid_exchange_receiver> result)
{
    result = mtk::make_cptr(new mtk::handle_qpid_exchange_receiver (make_cptr( new mtk::__internal_impl_handle_qpid_exchange_receiver(key._0, key._1, key._2, key._3))));
    return result;
}
...
class __internal_impl_handle_qpid_exchange_receiverMT   :  public mtk::SignalReceptor {
    mtk::non_copyable nc;

    public:
        explicit __internal_impl_handle_qpid_exchange_receiverMT(const t_qpid_url& url, const t_qpid_address& address, const t_qpid_filter& filter, const t_qpid_exch_recept_conf&  recept_conf);
        ~__internal_impl_handle_qpid_exchange_receiverMT(void)
        {
            try{
        ...
        mtk::nullable<mtk::DateTime>                  depreciated_on;
        Signal<const MESSAGE_TYPE&>                   signalMessage;
...
template<typename MESSAGE_TYPE>
class handle_qpid_exchange_receiverMT  {
    public:
        explicit handle_qpid_exchange_receiverMT(const t_qpid_url& url, const t_qpid_address& address, const t_qpid_filter& filter, const t_qpid_exch_recept_conf&  recept_conf)
            : __impl( new __internal_impl_handle_qpid_exchange_receiverMT<MESSAGE_TYPE> {url, address, filter, recept_conf})
        {
        }
        ~handle_qpid_exchange_receiverMT(void)
        {   mtk::delete_later(__impl);  }



        CountPtr<__internal_impl_handle_qpid_exchange_receiverMT<MESSAGE_TYPE> >              __impl;
};
...
template<typename MESSAGE_TYPE>
inline __internal_impl_handle_qpid_exchange_receiverMT<MESSAGE_TYPE>::__internal_impl_handle_qpid_exchange_receiverMT(
                                                const t_qpid_url& url,
                                                const t_qpid_address& address,
                                                const t_qpid_filter& filter,
                                                const t_qpid_exch_recept_conf&  recept_conf)
    :     signalMessage{}
        , hqpid_receiver( get_from_factory<mtk::handle_qpid_exchange_receiver>(mtk::make_tuple(url, address, filter, recept_conf)) )
{
    ++mtk_qpid_stats::num_created_suscriptions();
    MTK_CONNECT_THIS(hqpid_receiver->__impl->signalMessage, on_message)
}


template<typename MESSAGE_TYPE>
inline void __internal_impl_handle_qpid_exchange_receiverMT<MESSAGE_TYPE>::on_message(const qpid::messaging::Message& message)
{
    //  get control fields
