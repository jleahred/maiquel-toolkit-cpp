...
__internal_impl_handle_qpid_exchange_receiver::__internal_impl_handle_qpid_exchange_receiver(   const t_qpid_url& url,
                                                                                                const t_qpid_address& address,
                                                                                                const t_qpid_filter& filter,
                                                                                                const t_qpid_exch_recept_conf&  recept_qpid_config)
    :     signalMessage{}
        , receiver(get_from_factory<mtkqpid_receiver>(mtk::make_tuple(url, address, filter, recept_qpid_config)))
{
...
void check_NORMAL_queue(void)
{
    ...
    if(messages_per_check_interval < 0)
    {
        ...
        {
            messages_per_check_interval = 6;       //  300 msg/s
            check_interval = mtk::dtMilliseconds(16);       //  timer is called every 8ms  aprox
        }
        else
        {
