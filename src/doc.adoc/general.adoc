...
There are many styles codding C++ (Scott Meyers, standard library, Bijarne Stroustroup, Thinking in C++, Google, Qt...) +
...
* To help writing with code completion, try to group classes and functions by prefix-name. It will be easier to find your function with code-completion +
...
* Soft realtime applications, are applications that has to work fast and stable
* A good way to deal with it, is processing messages
** Every message has to be processed fast
...
.. Using ++mtk::CountPtr++ is difficult to have invalid memory access and pointer errors
.. Using ++mtk::Singal++ simplify the resource management
...
*** It's dificult to have a +mtk::CountPtr+ pointing to invalid address or not initialized memory
...
*** Random iterators
*** Invalid iterators
** There are +mtk+ containers with checked iterators (very strict checks, more than necessary) <<mtk_containers>>
...
* Some libraries need different options like (firebird or yaml) +
...
| mtk  | link to +mtk+ library folder
...
. Decompress the file (with command previously supplied)
. Execute +make all+ +
  This command will compile the library and your program
. Adjustments...
...
* +mtk+ is oriented to work with several processes integrated with a middleware
* Therefore, the concurrency model will be based on message communication
* Concurrency systems based on shared memory, are not scalable and very difficult maintenance
* When working with a lib with threads, we can use +MTK_SYNCHR+  as an easy and efficient solution (look on +support.synchr+)


=== Examples and regresion test

* On folder +examples+
* Most of them will write on standard output lines will be used as regression test
* Output not valid for regression test, will start with +NO_REGRESION+ (as time intervals, current dates, etc...)
* If there are any problem executing the example, it will return a code different to 0
* The library is verified with +valgrind+ +
  fbclient doesn't deallocate all the memory, therefore, examples and applications using it, will have a valgrind report with issues
