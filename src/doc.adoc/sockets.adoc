...
-------------
+
* Different levels can be combined "al gusto"
* They are not precombined in order to avoid a combinatory explosion

=== Sockets low level
--
* On this level we have the SO interface
* Here we can choose:
...
* Or perhaps a server mono-connection with locking write using binary messages with size indicator
* Or a socket for text with delimiter, server one connection managed with so signals
...
* With this kind of clasification, we could have hundreds or thousands of classes to combine all options
* Instead of creating all options before using it, the proposal is to pick functionalities on each level, and create your own class with this combination of functionalities
...
-----
+
* This model lets us reuse code creating classes when we need it with the specific functionality (combination of levels) avoiding the problem of super_complex_classes or combinatory explosion
