...
This is a one2one communication



* Secuence
.. Multiresponse subscription
.. Send request
.. Receiving responses (with sequence number)
...
* In case of not receiving the next response (timeout) or we will receive an incorrect partial response (bad sequence number), the system will cancel the listener to the response and will emit an alarm
...
-----
<1> This is the method that will receive the list with full response messages
...
<4> Now we tell the system to send all responses +
    They will be sent asynchronously and not very fast
+
* In case of no response, the system will remove the resources (subscription and object)
* The system will also check the sequences on multiresponses
