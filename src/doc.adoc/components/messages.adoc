...
* Same routing keys preceded by +SYS.+
--
...
* They will work after +tree_server+. The +tree_server+ will check the permisions for each product info request.
...
node [shape=box width=1.7] CLIENT;
node [shape=circle, width=1.5] tree_server1 product_loader;

CLIENT ->  tree_server1  [label = req_prod_info ]
tree_server1 ->  product_loader  [label = req_prod_info ]
...
digraph dfd0_graph {
 ...
 CLIENT -> tree_server [label = "req_prod_info" ];

 tree_server -> CLIENT [label = "res_tree_items" ];
 tree_server -> CLIENT [label = "pub_tree_colapse_branch" ];
 tree_server -> product_loader  [label = "req_init_prod_info" ];
 tree_server -> product_loader [label = "req_prod_info\n(cli)" ];

 product_loader -> tree_server   [label = "ps_pub_prod_info_mtk_ready" ];
