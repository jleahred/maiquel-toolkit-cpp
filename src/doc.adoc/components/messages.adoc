...
[cols="1, 1, 3" , options=""]
|=====================
| ALL.<gs_name>.#  |  GS->ALL | This messages will be sent to all servers (included TS) +
...
| GS.#             |  ALL->GS | comunication to +GS+ from other servers
| MK.<market>.#    |  GS->MK  | Market is used to filter for specific server +
                                ps_conf, ps_req, ...
| SYS.#            |  GS->SYS |  Internal messages to spread on +SYS+ level
|=====================
...
| MK.<client_code>.#   |  MK->CLI |   Messages from clients and +GS+  generated +
                                      <client_code> will be filtered con +CS+
| MK.(OMS\|PS).#        |  MK->CLI |   Messages to market servers generated on +SYS+
...
GS.#;;
* Messages generated on clients or servers to +GS+
+
