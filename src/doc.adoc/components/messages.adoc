...
| GS.<broler_code>.#             |  ALL->GS  |  communication to +GS+ from other servers
...
// * +CONF+ to send messages to many but they depend from any request or previous subscription information and the subscription is  _permanent . +
//   Orders confirmation for example
// * +REQ+ is for request responses and request confirmation patterns
// * +REP+ responses of request response pattern. _Non permanent_ subscriptions
...
* Information starts on market connections and is distributed to terminals
* i.e. prices
...
SYS.<broker_code>.#;;
* Messages generated on +GS+ or servers to +SYS+ (only one jump)
+
...
subgraph cluster_cimd {
   ...
   subgraph cluster_system {
          ...
          MK -- { GS1, GS2 }    [dir=back, style=dotted];
          MK -- { GS1, GS2 }    [dir=forward, style=dotted];
...
* This behavior lets to configure several +tree_servers+ for different users, having the possibility to show different trees to different users
+
...
* Online trading +stop market+ Orders client messages
+
.msg_trd_cli_sm.msg
-----
include::../../../src/components/trading/msg_trd_cli_sm.msg[]
-----


* Orders STOP MARKET status diagram
+
["graphviz", "components_trading_o_sm.png"]
-----------
include::../../../src/components/trading/trd_cli_sm.dot[]
------------
...
.msg_trd_cli_sl.msg
-----
include::../../../src/components/trading/msg_trd_cli_sl.msg[]
-----


* Orders STOP LIMIT status diagram
+
["graphviz", "components_trading_o_sl.png"]
-----------
include::../../../src/components/trading/trd_cli_sl.dot[]
------------
...
* Trading Limit Orders OMS messages (added permission filter fields)
. msg_trd_oms_rq.msg
+
-----
...
* This behavior gives the opportunity to a different +tree_server+ instance, to check and send response (if so). In this case, +tree_servers+ will work in parallel
...
* When a +tree_server+ detects new products on tree, it will send a collapse branch (where new products were inserted)
+
...
* They will work after +prodinfo_grants+. The +prodinfo_grants+ will check the permissions for each product info request.
...
** This message gives the opportunity to market price server, to start sending initialization (i.e.)
* This process will listen to +pub_market_pubisher_ready+, to send +req_init_prod_info+ to publisher
+
.DFD  request response product info
...
prodinfo_grants ->  product_loader  [label = ps_req_prod_info ]
...
digraph dfd0_graph {
 ...
 tree_server -> product_loader  [label = "ps_req_init_prod_info\n__to_publisher" color=red];
 prodinfo_grants -> product_loader [label = "ps_req_prod_info\n(cli)" ];
 ...
 MARKET  ->  product_loader   [label = "ps_pub_prod_info_mtk_ready\n__from_publisher" ];

 MARKET  ->  product_loader   [label = "ps_conf_full_product_info_init\n__from_publisher\n\nps_conf_full_product_info_init\n__from_publisher" ];

 MARKET  ->  conect_market_2_all   [label = "(cli) pub_(marg,depth,trades...)" ];
 MARKET  ->  conect_market_2_all   [label = "(cli) pub_product_stopped" color=red];
 MARKET  ->  conect_market_2_all   [label = "(cli) pub_market_stopped"color=red ];
 MARKET  ->  conect_market_2_all   [label = "(cli) pub_new_products" color=red];
...
digraph dfd0_graph {
    ...
    prod_load0 ->  none0  [label = ps_req_init_prod_info__to_publisher];
    none0  ->  market0 ;
    market0 -> prod_load1 [label = ps_pub_prod_info_mtk_ready__from_publisher];
    prod_load1 ->  market1  [label = ps_req_init_prod_info__to_publisher];
    market1  -> all [label = "ps_conf_full_product_info_init__from_publisher\nps_conf_full_product_info__from_publisher*"];
}
-----------
