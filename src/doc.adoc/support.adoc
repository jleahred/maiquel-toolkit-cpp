...
* Here we have some functions of general purpose
...
* In C/C++ es frequent an abnormal program stop due to a integer division by 0
...
* Very useful as non reversible function
...
* This is a template to work with pointers without pointers disadvantages and +RAII+
* An important difference with solutions like _boost_ or _c++0x_ is...
...
* Elements inside tuples has to be copyable
...
* There are two wrapper
nullable::
Requieres copy constructor and copy assignation operator
Nullable::
This one is more eficient, but requires empty constructor
...
** It's easy to represent not valid value for all platforms
** Comparision operators can be used. +
   If difference between doubles are very small, +mtk::double+ will identify as equal
...
* All classes and functions has the prefix +dt+
...
** You can subtract two +DateTime+ and you will get a +TimeQuantity+
...
* To avoid problems with parameters order, instead of using +int+, it's defined +dtMonth+, +dtHour+, ...
...
** This function uses a trick in order to work with 64bits and remove the problem of 49.7 days limit on 32bits
* Examples
+
.Example DateTime1
[source,{cpp}]
...
* Here we have a set of macros to control the max frequency execution of a block of code
MTK_EXEC_MAX_FREC::
...
If so, probably the program doesn't work as you expected and will be difficult to detect this issue

* There is a family +MTK_EXEC_MAX_FREC_NO_FIRST+ with same behavior except (as the name says) the first call, will not be executed
* You can nest them using the family +MTK_EXEC_MAX_FREC_A+
* This macros are very useful to avoid excessive messages on monitoring or logs
+
...
** This is not good for critical applications [red]#pending link#
** There are not verification on _deref_
** The value could be gone
*** Cleared container
*** Container out of scope
** Iterator end() doen't check anything and you can get a beatiful verse
** You can do ++end()
** How to change beatiful verses using iterators by exceptions?
...
*** An iterator is not valid when timestamp last container modification is different to timestamp saved on iterator
** Risk managed
*** Get a value from a not initialized iterator
*** Next iterator with not initialized iterator
...
* Safe comparision
* Save operations
* +fnExt+ is a support class witch manage number of decimals and increment
* You can add or subtract ticks to a +FixedNumber+
+
...
** Calling disconnect will disconnect only one of them
** It's possible to remove this behavior in the future
* It's very frequent to connect a signal with a method on current object. You can do it using +MTK_CONNECT_THIS+  see <<ex_mtk_connect_this, ex_mtk_connect_this>>
...
async_signal_all::
* It will re-throw all signals with the maximum speed configured on constructor
+
...
* It will re-throw an asynchronous signal with a list with all emits accumulated
+
.Example ex_async_signal_all_list
[source,{cpp}]
----
include::../../examples/onefile/support/ex_async_signal_all_list.cpp[]
----
--
...
** ++"concatenating" + var++ will work if var is +std::string+, but you will get wrong results (even beatiful verses) if var is a number or char* or any other pointer
* The proposal is to write... +MTK_SS("concatenating" << var)+
* It will work always as expected without problems or surprises

Converting to string  MTK_SS::
* Having several functions to convert to string is not comfortable and tedious
...
* To work with +MTK_SS+ is necessary to work with types with operator +<<+ overloaded for std streaming (not very difficult requirement)
+
[WARNING]
If +MTK_SS+ is used recursivily, you have to add a literal parameter in order to remove a gcc warning +
...
** If conversion works, first element will be converted and second element true
...
* +s_split+
** This function will return a +vector<string>+ splitting the string with the specified char
* +s_toLower  s_toUpper+
...
* This is not to save large data
* The parser doesn't work in stream mode, it will load all the file, will create the AST, will cut branches...
* To save huge datas, use a database or +yaml+ [red]#link pending#
* Access to config files has not been frequent
...
* Keep the manual comments and carry returns
* Keep +nodes+ and +properties+ order
* On save, the system will format the file
** Brakes alone in one line and properly indented
** It will remove spaces between properties and values
* This class works only with strings
* Reading will return +nullable+ type to make a difference between is empty and there is no value
* There is supported substitution
** A substitution is defined by +$(<property_to_replace>)+
...
* If a property is written empty, this property will be deleted
* If a property doesn't exist, a null nullable will be returned
* *Variables*
** When the value of a property has a text closed by +$( )+, this text will be replaced by the value of the property written inside
** If it's not possible, an exception will be raised
...
* *LISTS*
** A list is a property but values are closed with +[ ]+ and each element is in a different line
** Inside a list, you cannot write line comments neither end of line comments
** Empty lines are part of list (empty element)
** List elements will not have spaces at the beginning (all left spaces are removed on reading)
...
--
* It will check fluctuations between two consecutive messages and five minutes interval (with dynamic adjustment)
...
--
* There are three timers with different frequencies (10, 100, 1000 milliseconds)
* Each one will emit a signal
* You can connect your function or method to the appropriate signal
...
--
* Used to program a call to a method or function after x time
* It will produce only one call
* You can specify the frequency and precision of the call
* You don't have to use it to produce asynchronous calls, for that, try with +AsyncDelayedCall+
* The function/method called 
** Has to return a void
** Has to receive a parameter as a const reference +
   If you don't need it, declare a +int+ param and configure with 0
** The parameter type, has to be copyable
--
...
(mtk::dtSeconds(5), 11, CallAfter5s)  | calling to a object method
...
--
* It converts a method or function to work in asynchronous and delayed way 
* The method to transform has to:
** Return a void
** Receive only one parameter
** The param, has to be copyable
** If you need to receive more params, try with tuples
* The transformed method, will return immediately without doing his job at this moment (synchronous vs asynchronous)
...
* On field +sourceCode+ from +mtk::Alarm+ will be written the method converted name
* All will be executed on same thread
* It uses +mtk::Timer+ with precision of 100 milliseconds
* It could be applied in...
** Writing on db
** Sending alarms by network
** Sending responses to monitoring system
** Writing in log
** In general, in any case when is not critical an immediate response and we don't want to stop others activities more critical
* There are macros to combine declaration and implementation or doing it separately
** If you split declaration and implementation and parameters doesn't match, it will not compile (and I'm not responsible of error messages from compiler)
--
Params::
+
-----
...
TODO::
* Having a way to request for discarded or error messages, exceptions, overflows
* Configure a maximum frequency of error notifications. This speed has to bee lower than processing messages and it will help to avoid infinite loops
* The call created by macro (witch will be call asynchronously to do the work) would have to be constant. +
  For that, +mtk::SignalSlot+ has to work with +const+ methods
* Extend the macros to support functions
* Checking parameters to detect an cancel too aggressives ones
...
<3> This method will trap async errors
<4> Here is protected the original or normal method name (as documentation)
<5> Here is the same transformed to work asynchronously
<6> End implementation of async method
<7> Here we call several times very fast +
    Look that we call more than maximum configured
