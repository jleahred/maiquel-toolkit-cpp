...
* Using some +gcc+ tricks, we can build a partial solution for iterating all elements of a container
* With _C++0x_  it will not be necessary (and we will have a full solution)
* This is not very usefull because it's not supported by <<mtk_containers,mtk_containers>>
+
.Example MTK_FOREACH
[source,{cpp}]
----
include::../../examples/onefile/support/ex_foreach.cpp[]
----
+
.foreach.hpp
[source,{cpp}]
----
include::../../src/support/foreach.hpp[]
----
