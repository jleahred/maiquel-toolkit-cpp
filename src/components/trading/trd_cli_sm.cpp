...
#define CHECK_REQUEST_AND_WRITE_LAST_REQ \
    {   \
        ...
        tresult = mtk::trd::msg::check_request_request<mtk::trd::msg::RQ_XX_SM>   (rq, last_request     ()   ); \
        nerrors += tresult._0;    \
        ...
        (void)nerrors;   \
        (void)qty_lower_exec;   \
        ci->set_last_request(mtk::make_nullable( static_cast<const mtk::trd::msg::RQ_XX_SM&>(rq))); \
    }
...
#define CHECK_REQUEST_MOD_AND_WRITE_LAST_REQ \
    {   \
        ...
        ci->__serrors += tresult._1;    \
        mtk::tuple<int, std::string, bool> tresult2;    \
        tresult2 = mtk::trd::msg::check_request_last_confirm<mtk::trd::msg::RQ_XX_SM, mtk::trd::msg::CF_XX_SM>(rq, last_confirmation()   ); \
        nerrors += tresult2._0;    \
        ci->__serrors += tresult2._1;    \
        qty_lower_exec = tresult2._2;    \
        tresult = mtk::trd::msg::check_request_not_modifying<mtk::trd::msg::RQ_XX_SM, mtk::trd::msg::CF_XX_SM>(rq, last_request(), last_confirmation() ); \
        nerrors += tresult._0;    \
        ci->__serrors += tresult._1;    \
        (void)nerrors;   \
        ci->set_last_request(mtk::make_nullable( static_cast<const mtk::trd::msg::RQ_XX_SM&>(rq))); \
...
#define CHECK_CONFIRM_AND_WRITE_LAST_CONF \
    {   \
        ...
        nerrors += tresult._0;    \
        ci->__serrors += tresult._1;    \
        tresult = mtk::trd::msg::check_confirm__last_confirm<mtk::trd::msg::CF_XX_SM>(cf, last_confirmation()); \
        nerrors += tresult._0;    \
        ci->__serrors += tresult._1;    \
        (void)nerrors;    \
        ...
        ci->set_last_confirmation(mtk::make_nullable(static_cast<mtk::trd::msg::CF_XX_SM>(cf)));    \
    }
...
#define CHECK_REJECT_AND_WRITE_LAST_CONF \
    {   \
        ...
        ci->__serrors += tresult._1;    \
        (void)nerrors;  \
        tresult = mtk::trd::msg::check_reject__last_confirm<mtk::trd::msg::CF_XX_SM>(rj, last_confirmation());\
        nerrors += tresult._0;    \
        ci->__serrors += tresult._1;    \
        (void)nerrors;    \
        auto total_execs_emtpy = mtk::trd::msg::__internal_get_default((mtk::trd::msg::sub_total_executions*)0);  \
        ...
        ci->set_last_confirmation(mtk::make_nullable(static_cast<mtk::trd::msg::CF_XX_SM>(rj)));    \
    }


#define IF_LAST_CONF_ISNULL__WRITE_IT  \
        if(ci->last_confirmation().HasValue() == false)      \
            ci->set_last_confirmation(mtk::make_nullable(static_cast<mtk::trd::msg::CF_XX_SM>(st)));
...
#define CREATE_AND_SEND_REJECT(__SIGNAL_TYPE__, __MSG_TYPE__, __DESCRIPTION__) \
    ...
    {               \
        ...
        mtk::trd::msg::CF_XX_SM  rjxx(cfxx, last_confirmation().Get().market_pos);      \
        cptr_rj = mtk::make_cptr(new mtk::trd::msg::__MSG_TYPE__(mtk::trd::msg::RJ_XX_SM(rjxx, last_request().Get().request_pos))); \
    ...
    {      \
        ...
        {      \
            ...
            mtk::trd::msg::CF_XX_SM  rjxx(cfxx, last_request().Get().request_pos);      \
            cptr_rj = mtk::make_cptr(new mtk::trd::msg::__MSG_TYPE__(mtk::trd::msg::RJ_XX_SM(rjxx, last_request().Get().request_pos))); \
            ci->__SIGNAL_TYPE__(*cptr_rj);  \
        }    \
        else     \
            mtk::AlarmMsg(mtk::Alarm("SEND_REJECT", "trd_cli_sm.cpp", MTK_SS("Missing last_request trying to reject "), mtk::alPriorCritic, mtk::alTypeNoPermisions));    \
    }   \
...
#define  ADD_HIST_RQ(__VAR_NAME__, __nw_md_cc__)\
    {       \
        ...
        std::string result_additem = ci->history()->add_item(order_historic_item({false, tt_rq_pending, tt2_##__nw_md_cc__,        \
                        ...
                        __VAR_NAME__.request_pos.cli_ref, "", MTK_SS("stop_price:" << __VAR_NAME__.request_pos.stop_price)}));       \
        if(result_additem != "")        \
        {       \
...
#include "components/trading/trd_cli_sm.impl"           //-----------------------------------------------------------
