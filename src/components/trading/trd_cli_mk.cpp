...
mtk::tuple<int, std::string> check_request_request(const mtk::trd::msg::RQ_XX_MK& rq, const mtk::nullable<mtk::trd::msg::RQ_XX_MK>& last_request)
{
    std::string serrors;
    int nerrors=0;
    ...
    {
        mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "trd_cli_mk.cpp", MTK_SS(serrors << "  " << rq << " / " << last_request), mtk::alPriorCritic, mtk::alTypeNoPermisions));
        ++nerrors;
    }
...
mtk::tuple<int, std::string> check_request_last_confirm(const mtk::trd::msg::RQ_XX_MK& rq, const mtk::nullable<mtk::trd::msg::CF_XX_MK>& last_conf)
{
    ...
    if (nerrors >0)
    {
        mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "trd_cli_mk.cpp", MTK_SS(serrors << "  " << rq << " / " << last_conf), mtk::alPriorCritic, mtk::alTypeNoPermisions));
        ++nerrors;
    }
    ...
    if (last_conf.HasValue()  &&  mtk::Double(last_conf.Get().confirmated_info.total_execs.quantity.GetDouble()) >= (mtk::Double(rq.request_pos.quantity.GetDouble())))
    {
        ...
        ++nerrors;
        mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "trd_cli_mk.cpp", MTK_SS(serrors << "  " << rq << " / " << last_conf), mtk::alPriorWarning, mtk::alTypeNoPermisions));
    }
    
    return mtk::make_tuple(nerrors, serrors);
}

mtk::tuple<int, std::string> check_request_not_modifying(const mtk::trd::msg::RQ_XX_MK& rq, const mtk::nullable<mtk::trd::msg::RQ_XX_MK>& last_request, const mtk::nullable<mtk::trd::msg::CF_XX_MK>& last_conf)
{
    if (last_conf.HasValue() == false)     return mtk::make_tuple(0, std::string());
    ...
    if (last_request.HasValue()== false  ||  last_request.Get().req_info ==  last_conf.Get().req_info)         //  there are no pending request
        if (rq.request_pos == last_conf.Get().confirmated_info.market_pos  &&  rq.cli_ref == last_conf.Get().confirmated_info.cli_ref)
            mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "trd_cli_mk.cpp", MTK_SS("modification not modifing  " << "  " << rq << " / " << last_conf), mtk::alPriorWarning, mtk::alTypeNoPermisions));
...
mtk::tuple<int, std::string> check_confirm_request(const mtk::trd::msg::CF_XX_MK& cf, const mtk::nullable<mtk::trd::msg::RQ_XX_MK>& last_request)
{
    std::string serrors;
    int nerrors=0;
    ...
    if (nerrors >0)
    {
        mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "trd_cli_mk.cpp", MTK_SS(serrors << "  " << cf << " / " << last_request), mtk::alPriorCritic, mtk::alTypeNoPermisions));
        ++nerrors;
    }
    return mtk::make_tuple(nerrors, serrors);
}
...
mtk::tuple<int, std::string> check_confirm__last_confirm(const mtk::trd::msg::CF_XX_MK& cf, const mtk::nullable<mtk::trd::msg::CF_XX_MK>& last_conf)
{
    if (last_conf.HasValue() == false)     return mtk::make_tuple(0, std::string());
    ...
    if (nerrors >0)
    {
        mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "trd_cli_mk.cpp", MTK_SS(serrors << "  " << cf << " / " << last_conf), mtk::alPriorCritic, mtk::alTypeNoPermisions));
        ++nerrors;
    }
    return mtk::make_tuple(nerrors, serrors);
}
...
mtk::tuple<int, std::string> check_reject_request(const mtk::trd::msg::RJ_XX_MK& rj, const mtk::nullable<mtk::trd::msg::RQ_XX_MK>& last_request)
{
    std::string serrors;
    int nerrors=0;
    ...
    if (nerrors >0)
    {
        mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "trd_cli_mk.cpp", MTK_SS(serrors << "  " << rj << " / " << last_request), mtk::alPriorCritic, mtk::alTypeNoPermisions));
        ++nerrors;
    }
    return mtk::make_tuple(nerrors, serrors);
}
...
mtk::tuple<int, std::string> check_reject__last_confirm(const mtk::trd::msg::RJ_XX_MK& rj, const mtk::nullable<mtk::trd::msg::CF_XX_MK>& last_conf)
{
    if (last_conf.HasValue() == false)     return mtk::make_tuple(0, std::string());
    ...
    if (nerrors >0)
    {
        mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "trd_cli_mk.cpp", MTK_SS(serrors << "  " << rj << " / " << last_conf), mtk::alPriorCritic, mtk::alTypeNoPermisions));
        ++nerrors;
    }
    return mtk::make_tuple(nerrors, serrors);
}
...
mtk::tuple<int, std::string>  check_exec__last_confirm(const mtk::trd::msg::CF_EX_MK& ex, const mtk::nullable<mtk::trd::msg::CF_XX_MK>& last_conf)
{
    if (last_conf.HasValue() == false)    
    {
        std::string serrors = MTK_SS("execution received on non confirmated order  " << ex << " / " << last_conf);
        mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "trd_cli_mk.cpp", serrors, mtk::alPriorCritic, mtk::alTypeNoPermisions));
    ...
    if (nerrors >0)
    {
        mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "trd_cli_mk.cpp", MTK_SS(serrors << "  " << ex << " / " << last_conf), mtk::alPriorCritic, mtk::alTypeNoPermisions));
        ++nerrors;
    }
    return mtk::make_tuple(nerrors, serrors);
}
...
#define SEND_REJECT(__SIGNAL_TYPE__, __MSG_TYPE__) \
    if (last_confirmation().HasValue())    \
        ci->__SIGNAL_TYPE__(mtk::trd::msg::__MSG_TYPE__(mtk::trd::msg::RJ_XX_MK(mtk::trd::msg::RJ_XX(rq.req_info, rq.product_code, MTK_SS("Error: Received rq_md on status " << GetStatusName()), \
                        mtk::admin::get_control_fluct_info()), last_confirmation().Get().confirmated_info)));  \
    else      \
    {      \
        if (last_request().HasValue()==true)      \
        {      \
            mtk::trd::msg::RQ_XX_MK  _last_request = last_request().Get();  \
            mtk::trd::msg::sub_order_mk_confirmated to_confirm(mtk::trd::msg::sub_order_xx_confirmated( _last_request.order_id,     \
            ...
            ci->__SIGNAL_TYPE__(mtk::trd::msg::__MSG_TYPE__(mtk::trd::msg::RJ_XX_MK(mtk::trd::msg::RJ_XX(rq.req_info, rq.product_code, MTK_SS("Error: Local reject. Order not sent. Received rq_md on status " << GetStatusName()), \
