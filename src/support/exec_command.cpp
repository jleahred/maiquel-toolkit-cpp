...
exec_command::exec_command(const std::string& _description, const mtk::dtTimeQuantity&  _time_out)
        :   description(_description), child_pid(0), time_out(_time_out), exit_info({false, ""})
{
    p_child2parent[0]=0;
    p_child2parent[1]=0;
    p_parent2child[0]=0;
...
exec_command::~exec_command(void)
{
    close_pipe(p_child2parent);
    close_pipe(p_parent2child);
    close_pipe(p_stderror);

    if(child_pid!=0)
        mtk::AlarmMsg(mtk::Alarm(MTK_HERE, "~exec_command", "destroying with running command", mtk::alPriorError));
    __internal_stop();
}
...
void exec_command::run(void)
{
    ...
    close_pipe(p_child2parent);  close_pipe(p_parent2child);   close_pipe(p_stderror);
    if (pipe(p_child2parent) != 0 || pipe(p_parent2child) != 0  || pipe(p_stderror) != 0)
    {
        throw mtk::Alarm(MTK_HERE, "exec_command", MTK_SS("Error creating pipes command  " << command << "  errno"
                                << errno << "  "  <<  strerror(errno)), mtk::alPriorError);
    }
...
void  exec_command::__check_output(void)
{
    ...
    //if(child_pid  &&  kill(child_pid,0))
    {
        ...
        if(child_pid!=0)
        {
            ...
            signal_terminated.emit(exit_info._0, description, exit_info._1, mtk::dtNowLocal() - start_running.Get());
            start_running = {};
        }
        return;
    }
